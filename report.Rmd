...
The plot shows average hourly delays for buses, streetcars, and subways after removing outliers such as severe disruptions. Buses exhibit the largest variability, with a sharp peak around 3–4 AM and a steady rise in the evening, likely reflecting overnight disruptions and evening congestion. Streetcar delays remain more stable but slightly elevated, reflecting their exposure to mixed traffic. Subway delays are the most consistent and lowest throughout the day, suggesting greater resilience to surface-level disruptions.
...
transit_data_filtered <- transit_data %>%
  ...
  group_by(Source) %>% 
  mutate(Q1 = quantile(min_delay, 0.25, na.rm = TRUE),
         Q3 = quantile(min_delay, 0.75, na.rm = TRUE),
         IQR = Q3 - Q1,
         Lower_Bound = Q1 - 1.5 * IQR,
         Upper_Bound = Q3 + 1.5 * IQR) %>%
  filter(min_delay >= Lower_Bound & min_delay <= Upper_Bound) %>%
  ungroup()
...
This heatmap illustrates average hourly delays by transit mode. Bus delays are consistently the highest, with delays intensifying into the evening. Streetcar delays remain moderate and relatively stable, while subway delays are the lowest and most uniform throughout the day. The gradient highlights how surface modes—particularly buses—are more affected by hourly fluctuations, likely due to road traffic and service frequency.
